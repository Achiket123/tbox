...
func RunContainer(cfg Config) (int, error) {
	...
	// tmpDir starts with /data/data..., so we strip the leading slash for filepath.Join
	overlayTmpDir := filepath.Join(overlayPath, strings.TrimPrefix(tmpDir, "/"))
	fmt.Printf("[DEBUG] tmpDir: %s\n", tmpDir)
	fmt.Printf("[DEBUG] overlayPath: %s\n", overlayPath)
	fmt.Printf("[DEBUG] overlayTmpDir: %s\n", overlayTmpDir)

	if err := os.MkdirAll(overlayTmpDir, 0755); err != nil {
		return -1, fmt.Errorf("create overlay proot tmp dir: %w", err)
	}

	if info, err := os.Stat(overlayTmpDir); err == nil {
		fmt.Printf("[DEBUG] Stat confirmed overlayTmpDir exists, isDir: %v\n", info.IsDir())
	} else {
		fmt.Printf("[DEBUG] Stat failed after MkdirAll! err: %v\n", err)
	}
	...
	os.MkdirAll(overlayPrefixTmp, 0755)
...
func buildProotArgs(overlayPath string, cfg Config) []string {
	...
	// L3 fix: --kill-on-exit is NOT a valid flag in Termux's standard proot
	// package (it exists only in some proot-distro forks). Including it causes
	// proot to exit immediately with "unrecognized option". Orphan processes
	// are handled by the signal forwarding in main.go instead.
